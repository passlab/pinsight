...
output_file("performance_plot.html")
...
if __name__ == "__main__":
    ...
    if is_per_region:
        ...
        source = ColumnDataSource(data=dict(
            ...
            work=[(int(t) - int(o)) / 1000000 for (t, o) in zip(data["total_execute_time"], data["total_overhead"])],
            overhead=[int(x) / 1000000 for x in data["total_overhead"]],
            idle=[int(x) / 1000000 for x in data["total_idle"]],
        ))
    else:
    ...
    if is_per_region:
        ...
        categories = ['work', 'overhead']
        colors = ['blue', 'red']

        r_work, r_overhead = p.vbar_stack(categories, x='x', width=0.9, alpha=0.5, color=colors, source=source)#,
                 #legend=[value(x) for x in categories])
    ...
    p.y_range.start = 0
    p.y_range.end = max([int(x) / 1000000 for x in data["total_execute_time"]])
