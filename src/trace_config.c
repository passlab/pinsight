...
void setup_trace_config_env() {
    ...
    for (int i = 0; i < num_domain; i++) {
        ...
        // Convert to uppercase "PINSIGHT_TRACE_" is 15 characters; thus start with 15 
        for(int j=15; env_var[j]; j++) env_var[j] = toupper((unsigned char)env_var[j]);
...
__attribute__ ((constructor)) void initial_setup_trace_config() {
	...
	lexgion_default_trace_config->tracing_rate = DEFAULT_TRACE_RATE; //trace every execution
	lexgion_default_trace_config->trace_starts_at = DEFAULT_TRACE_START; //start tracing from the first execution	
	lexgion_default_trace_config->max_num_traces = DEFAULT_TRACE_MAX; //unlimited traces
