 ...
 * names to real extern declarations via PINSIGHT_OPENMP (set by
...
/* these are thread-local storage assuming that OpenMP runtime threads are 1:1
 * mapped to the system threads (PThread for example). If not, we should
 * implement our own OpenMP thread-local storage (not system TLS).
 */
...
static const char *ompt_task_type_t_values[] = {
    NULL, "ompt_task_initial", "ompt_task_implicit", "ompt_task_explicit",
    "ompt_task_target"};
...
static void print_ids(int level) {
  ...
  int exists_task =
      ompt_get_task_info(level, NULL, &task_data, &frame, &parallel_data, NULL);
  if (frame) {
    if (debug_on) {
      printf("%" PRIu64 ": task level %d: parallel_id=%" PRIu64
             ", task_id=%" PRIu64 ", exit_frame=%p, enter_frame=%p\n",
             ompt_get_thread_data()->value, level,
             exists_task ? parallel_data->value : 0,
             exists_task ? task_data->value : 0, frame->exit_frame.ptr,
             frame->enter_frame.ptr);
    }
    //    printf("%" PRIu64 ": parallel level %d: parallel_id=%" PRIu64 ",
    //    task_id=%" PRIu64 ", exit_frame=%p, reenter_frame=%p\n",
    //    ompt_get_thread_data()->value, level, exists_parallel ?
    //    parallel_data->value : 0, exists_task ? task_data->value : 0,
    //    frame->exit_frame.ptr, frame->enter_frame.ptr);
  } else {
    if (debug_on) {
      printf("%" PRIu64 ": task level %d: parallel_id=%" PRIu64
             ", task_id=%" PRIu64 ", frame=%p\n",
             ompt_get_thread_data()->value, level,
             exists_task ? parallel_data->value : 0,
...
static void get_parallel_task_info() {
  ...
  if (retv == 2)
    printf("initial implicit parallel region is ready, team size: %d, parallel "
           "data: %p\n",
           team_size, parallel_data);
  else if (retv == 1)
    printf("ancestor exist, but the initial implicit parallel region is not "
           "ready\n");
  else
    printf(
        "system has not ready for the initial implicit parallel region: %p\n",
        parallel_data);
  ...
  int task_flag;
  int thread_num;
  retv = ompt_get_task_info(0, &task_flag, &task_data, NULL,
                            &task_parallel_data, &thread_num);
  if (retv == 2)
    printf("initial implicit task is ready, thread num: %d, parallel data: %p, "
           "task_data: %p\n",
           thread_num, task_parallel_data, task_data);
  else if (retv == 1)
    printf("ancestor exist, but the initial implicit task is not ready\n");
  else
    printf("system has not ready for the initial implicit task: %p of the "
 ...
 *
 * We manually set the codeptr_ra as 0x00000000000 for initial parallel region
 * (parallel_codeptr) and initial task The initial parallel lexgion is created
 * for this codeptr_ra address in this thread_begin event. This lexgion will be
 * at the bottom of the lexgion stack of each thread. From the spec, this
 * initial parallel region is the one enclosing the whole OpenMP program. Thus
 * technically, it should be created before the main program is entered. The
 * same for the initial task, which should be created when the main program
 * starts. We can implement this by calling the init_thread_data and
 * initializing the initial parallel region and initial task in the
 * enter_pinsight_func, which is before the OpenMP runtime and OMPT is setup.
 * This however looks wired. Thus in this implementation, we initialize the
 * initial parallel region in the thread_begin event, and the initial task in
 * the implicit_task callback. In this way, for the execution between when the
 * main program starts to the first OpenMP calls, they are not traced. This has
 * to be understood in order to show this execution as sequential portion of the
 * program execution in the analysis.
 *
 * For the initial task lexgion of the whole program, which is created in the
 * implicit_task callback, we set the codeptr_ra as 0x000000000, so far, we do
 * not know what it will be used for. But since it is part of the standard and
 * the OMPT implements this, we can add that lexgion in case it become useful
 * for the future.
 *
 * @param thread_type
 * @param thread_data
...
void on_ompt_callback_thread_begin(ompt_thread_t thread_type,
  ...
  // done here. The initial task is setup in the implicit_task callback
  if (thread_type == ompt_thread_initial) {
    // get_parallel_task_info();
    pinsight_thread_data.initial_thread = 1;

    // Setup the initial parallel region, and this could be a call to the
    // callback, but we will elaborate it here
    // on_ompt_callback_parallel_begin(NULL, NULL,
    // &pinsight_thread_data.lexgion_stack[0], 1, ompt_parallel_team ||
    // ompt_parallel_invoker_runtime, INITIAL_PARALLEL_CODEPTR);
    ...
    // NO NEED for setting up the initial task since it is setup by the
    // callback_implicit_task
  } else {
    pinsight_thread_data.initial_thread = 0;
  }

  // This is the codeptr for the first lexgion of each thread
  // lexgion_t * lgp = lexgion_begin(OPENMP_LEXGION, ompt_callback_thread_begin,
  // codeptr_ra);
}

#define PRINTF_LEXGION_HEADER                                                  \
  printf("#\t\tcodeptr\t\t\ttype\tcount\t\ttrace count(first-last)\n")
#define PRINTF_LEXGION_INFO(i, lgp)                                            \
  printf("%d\t%p-%p\t%d\t%d\t\t%d(%d-%d)\n", i, lgp->codeptr_ra,               \
         lgp->end_codeptr_ra == NULL ? (void *)NULL_DEFAULT_CODEPTR            \
                                     : lgp->end_codeptr_ra,                    \
...
void on_ompt_callback_parallel_end(ompt_data_t *parallel_data,
  ...
  /* Restore enclosing context when bookkeeping is active (MONITORING/TRACING).
   * parent_task->ptr may be NULL during mode switches where implicit_task
   * did not fire for workers in the current region. */
  if (PINSIGHT_DOMAIN_ACTIVE(
          domain_default_trace_config[OpenMP_domain_index].mode) &&
 ...
 * (flags & ompt_task_initial) evaluates to true for the initial implicit task
 * created from the initial implicit parallel region (flags &
...
void on_ompt_callback_implicit_task(ompt_scope_endpoint_t endpoint,
  ...
  case ompt_scope_begin: {
    if (flags &
        ompt_task_initial) { // For the initial implicit task of the initial
                             // parallel region: there is no parallel_begin
                             // event, thus we set the parallel_data here
                             // initial implicit task callback is fired here,
                             // not in callback_task_create
      parallel_data->ptr = (void *)enclosing_parallel_lexgion_record;
      // printf("callback_implicit_task fired for ompt_task_initial\n");
    }

    // These three setup are redundant for the main thread since they are
    ...
    /// Here a new lexgion with the same codeptr as the parallel region is
    /// created, but this lexgion has implicit_task type
  ...
  case ompt_scope_end: {
     ...
     * for the main thread. For other thread, we might not need this at all
     * since they will be setup when this thread is used later on
     */
    break;
  }
  }
...
void on_ompt_callback_work(ompt_work_t wstype, ompt_scope_endpoint_t endpoint,
  ...
  case ompt_scope_begin:
    if (codeptr_ra != parallel_codeptr && codeptr_ra != task_codeptr) {
      /* Option C: Only create a work lexgion if address-specific config exists
       */
  ...
  case ompt_scope_end:;
    ...
    if (work_has_own_lexgion) {
      ...
      /* Piggybacked on enclosing region, no lexgion to pop */
      lgp = top->lgp;
...
void on_ompt_callback_masked(ompt_scope_endpoint_t endpoint,
  ...
  switch (endpoint) {
  case ompt_scope_begin:;
    /* Option C: Only create a masked lexgion if address-specific config exists
     */
    lexgion_trace_config_t *addr_config =
        retrieve_lexgion_trace_config(codeptr_ra);
    if (addr_config) {
...
/**
 * In this callback, we will separate parallel_join sync from
 * barrier/taskwait/taskgroup sync
 *
 * @param kind
 * @param endpoint
 * @param parallel_data
...
void on_ompt_callback_sync_region(ompt_sync_region_t kind,
                                  ...
                                  ompt_data_t *task_data,
                                  const void *codeptr_ra) {
  /* Sync callbacks have no bookkeeping role (no lexgion_begin/end).
   * Only needed in TRACING mode for LTTng emission. */
  if (!PINSIGHT_SHOULD_TRACE(OpenMP_domain_index))
    return;
  if (task_data->ptr == NULL)
  ...
  case ompt_scope_begin: {
    switch (kind) {
#if COMPLETE_OPENMPRUNTIME_IMPLEMENTATION_4_BARRIER // until we have a runtime
                                                    // that fully support this
                                                    // as defined in the
                                                    // standard
    case ompt_sync_region_barrier_implicit_parallel:
    case ompt_sync_region_barrier_teams:
      printf("sync_region: codeptr: %p, parallel_code_ptr: %p, thread num: %d, "
             "trace_bit: %d\n",
    ...
    case ompt_sync_region_barrier_implicit_workshare:
      ...
      // barrier, thus we use the following workaround
    case ompt_sync_region_barrier: // barrier (implicit or explicit)
    case ompt_sync_region_barrier_explicit:
    case ompt_sync_region_barrier_implicit: // for join barrier and implicit
                                            // barrier such as at the end of a
                                            // for loop
    case ompt_sync_region_barrier_implementation:
    case ompt_sync_region_barrier_implicit_workshare:
    case ompt_sync_region_barrier_implicit_parallel:
  ...
  case ompt_scope_end: {
    ...
    case ompt_sync_region_barrier_teams:
      if (codeptr_ra == NULL || parallel_codeptr == codeptr_ra) {
         ...
         * NULL: non-master thread; if parallel_lgp->codeptr_ra == codeptr_ra:
         * master thread */
        /* this is NOT the actual join_end since by the time sycn region
         * terminates, a thread may already finish join_end, waiting in the pool
...
void on_ompt_callback_sync_region_wait(ompt_sync_region_t kind,
                                       ...
                                       ompt_data_t *task_data,
                                       const void *codeptr_ra) {
  /* Sync_region_wait has no bookkeeping role — only needed for TRACING. */
  if (!PINSIGHT_SHOULD_TRACE(OpenMP_domain_index))
    return;
  if (task_data->ptr == NULL)
    return; /* Mode transition: task_data not yet initialized */
  ...
  case ompt_scope_begin: {
    ...
    case ompt_sync_region_barrier_implicit_parallel:
    case ompt_sync_region_barrier_teams:
      if (codeptr_ra == NULL || parallel_codeptr == codeptr_ra) { // safety
                                                                  // check
        /* this is the join barrier for the parallel region: if codeptr_ra ==
         * NULL: non-master thread; if parallel_lgp->codeptr_ra == codeptr_ra:
         * master thread */
  ...
  case ompt_scope_end: {
    ...
    case ompt_sync_region_barrier_teams:
      if (codeptr_ra == NULL || parallel_codeptr == codeptr_ra) {
         ...
         * NULL: non-master thread; if parallel_lgp->codeptr_ra == codeptr_ra:
         * master thread */
        /* this is NOT the actual join_end since by the time sync region
         * terminates, a thread may already finish join_end, waiting in the pool
         * and then resummoned for doing the work, so this is actually when a
         * thread is about to enter into a new parallel region and start an
         * implicit task */
  ...
  case ompt_mutex_lock:
    if (debug_on) {
      printf("%" PRIu64 ": ompt_event_wait_lock: wait_id=%" PRIu64
             ", hint=%" PRIu32 ", impl=%" PRIu32 ", return_address=%p \n",
...
static void on_ompt_callback_cancel(ompt_data_t *task_data, int flags,
  ...
  if (debug_on) {
    printf("%" PRIu64 ": ompt_event_cancel: task_data=%" PRIu64
...
//  {
//    case ompt_scope_begin:
//      lttng_ust_tracepoint(ompt_pinsight_lttng_ust, global_thread_numle_begin,
//      global_thread_num, ompt_get_thread_data());
//      //printf("%" PRIu64 ": ompt_event_idle_begin:\n",
//      ompt_get_thread_data()->value);
//      //printf("%" PRIu64 ": ompt_event_idle_begin: global_thread_num=%"
//      PRIu64 "\n", ompt_get_thread_data()->value, thread_data.value); break;
//    case ompt_scope_end:
//      lttng_ust_tracepoint(ompt_pinsight_lttng_ust, global_thread_numle_end,
//      global_thread_num, ompt_get_thread_data());
...
on_ompt_callback_task_create(ompt_data_t *encountering_task_data,
  ...
  /** this is not useful anymore since the initial implicit task callback is
   * triggered as callback_implicit_task, not callback_task_create
   */
...
static void on_ompt_callback_dependences(ompt_data_t *task_data,
  ...
  if (debug_on) {
    printf("%" PRIu64 ": ompt_event_task_dependences: task_id=%" PRIu64
           ", deps=%p, ndeps=%d\n",
           ompt_get_thread_data()->value, task_data->value, (void *)deps,
           ndeps);
  }
}
