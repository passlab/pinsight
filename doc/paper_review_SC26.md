...
| INTROSPECT action (lttng rotate + script + sigsuspend) | ✅ Yes | ❌ Missing |
...
| Cyclic INTROSPECT with TRACING resume | ✅ Yes | ❌ Missing |
...
- No evaluation of the domain modes, INTROSPECT, knobs, or runtime reconfiguration features
- The recent knob and INTROSPECT evaluation work done on the 48-core machine is not included
...
- ⚠️ Doesn't mention domain trace modes, INTROSPECT, or knobs
...
- ❌ Should mention what new features (INTROSPECT, runtime config) address specific motivation gaps
...
- ⚠️ "In-Situ Performance Analysis" subsection mentions LTTng rotation but NOT the INTROSPECT mechanism which automates exactly this workflow
...
- ❌ **Missing**: INTROSPECT implementation details (arguably the most novel contribution)
...
- ❌ **No evaluation of**:
  ...
  - INTROSPECT feature end-to-end
  - Runtime reconfiguration (SIGUSR1 mode switching with LULESH)
  - Application knobs
- ⚠️ The in-situ analysis subsection (5.3) describes a debugger-based approach — but the INTROSPECT mechanism is a much more compelling automated version
...
- ❌ No mention of limitations or future directions for INTROSPECT, knobs, adaptive tuning
...
2. **Add a dedicated "System Design" or "Architecture" section** that presents the config system, domain modes, INTROSPECT workflow, and knob API as a cohesive design
...
- LULESH INTROSPECT evaluation (babeltrace2 verified: 792 events, cyclic TRACING mode)
...
[Lexgion.default]
    max_num_traces = 100
    tracing_rate = 10
    trace_mode_after = INTROSPECT:30:analyze.sh:MONITORING
[Lexgion(OpenMP).0x401234]
    max_num_traces = 500
```

### 3. Add a INTROSPECT workflow figure

The INTROSPECT execution flow (lttng rotate → fork+exec → sigsuspend → resume) would make an excellent figure and is arguably the most novel contribution vs existing tools.
